...
f"{fmt(p_3d)}", f"{fmt(ci_lower_10d)} - $`{fmt(ci_upper_10d)}",
...
kolumny = ["Data Wejścia", "Token", "Typ Sygnału", "Cena Wejścia ()", "SL ()", "Data Wyjścia", "Status", "Zysk (%)"]
...
prev_extr = float(row["Ekstremum ()"]) else entry
...
pa_desc = f"Aktywo znajduje się w wyraźnym trendzie wzrostowym, notując cenę powyżej kluczowej, 200-okresowej średniej kroczącej (EMA200: {fmt(ema_raw)}). Struktura ta definiuje strukturalny trend spadkowy, co znacząco zwiększa ryzyko dla pozycji typu long." else: pa_desc = f"Aktywo porusza się w fazie horyzontalnej konsolidacji, oscylując w pobliżu EMA200 ({fmt(ema_raw)} $``). Brak wyraźnego kierunku rynkowego sugeruje trwającą walkę popytu z podażą – zalecana jest ostrożność do momentu ostatecznego wybicia."
...
"Cena Wejścia ($)": round(cena_we, 5), "Cel TP (6%) ($)": round(cel_tp, 5),

"SL ($)": round(sl, 5), "Ekstremum ($)": round(cena_we, 5),
