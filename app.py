...
def calculate_altcoin_season_index():
    try:
        ...
        params = {
            ...
            "sparkline": "false", "price_change_percentage": "200d"
        }
...
def run_predictions(df_ta, fng_val):
    ...
    def analyze_row(row):
        ...
        shocks = rng.normal(loc=adjusted_drift, scale=vol_1h, size=(num_simulations, time_steps))
...
# ==========================================
# HISTORIA I AKTUALIZACJA POZYCJI (TYLKO LONG)
# ==========================================
HISTORY_FILE = "signals_history.csv"
...
col_t.title("📊 Analiza Krypto Spot (Multi-Timeframe)")
...
with tab2:
    ...
    chosen_token = col_sel_tok.selectbox("Wybierz token do otwarcia w archiwum:", df_ml["Token"].tolist(), key="manual_token_pick")
    ...
    if col_sel_btn.button("🚀 Dodaj do aktywnych pozycji", type="primary"):
        ...
        sig_text = row_data["Sygnał Hybrydowy"]
        
        new_entry = pd.DataFrame([{
            "Data": now_full, "Token": chosen_token, "Typ Sygnału": sig_text,
        ...
        st.success(f"Pomyślnie otwarto pozycję Long dla {chosen_token} po cenie ${fmt(price_clean)}!")
        st.rerun()
    ...
    if os.path.exists(HISTORY_FILE):
        try:
            ...
            if not df_active.empty:
                ...
                st.info("Brak aktywnych pozycji. Wybierz token w zakładce 'Sygnały Hybrydowe' i dodaj go ręcznie.")
        except Exception:
            st.info("Brak danych.")
    else:
        st.info("Brak otwartych pozycji. Archiwum jest puste.")
