...
def get_global_market_data():
    try:
        res = requests.get(
            ...
            headers={"User-Agent": "CryptoDashboardPro/2.0"}, timeout=4
        ).json()
...
def calculate_altcoin_season_index():
    try:
        ...
        params = {"vs_currency": "usd", "order": "market_cap_desc", "per_page": 60, "page": 1, "sparkline": "false", "price_change_percentage": "200d"}
        res = requests.get(url, params=params, headers={"User-Agent": "CryptoDashboardPro/2.0"}, timeout=6)
        ...
        for coin in coins:
            if coin["symbol"].lower() == "btc":
                btc_change = coin.get("price_change_percentage_200d_in_currency", 0.0) or 0.0
                break
...
def fetch_from_coingecko(gecko_id):
    ...
    res = requests.get(url, headers={"User-Agent": "CryptoDashboardPro/2.0"}, timeout=5)
    res.raise_for_status()
...
def get_simple_coingecko_price(gecko_id):
    try:
        ...
        res = requests.get(url, headers={"User-Agent": "CryptoDashboardPro/2.0"}, timeout=4).json()
        if gecko_id in res:
...
# ==========================================
# ULEPSZONY ALGORYTM SCORINGU I PREDYKCJI
# ==========================================
...
def update_history_status(df_ta):
    ...
    for idx, row in df_hist.iterrows():
        entry = float(row["Cena Wejścia"]) if pd.notna(row.get("Cena Wejścia")) else 0.0
        if entry <= 0: continue
        curr_price = float(price_map.get(row["Token"], entry))
        prev_extr = float(row["Ekstremum Ceny"]) if pd.notna(row.get("Ekstremum Ceny")) and float(row["Ekstremum Ceny"]) > 0 else entry
        ...
        if max_gain_pct >= 5.0 and str(row.get("TP 5%", "-")) == "-": df_hist.at[idx, "TP 5%"] = f"✅ {now_date}"
        if max_gain_pct >= 7.5 and str(row.get("TP 7.5%", "-")) == "-": df_hist.at[idx, "TP 7.5%"] = f"✅ {now_date}"
        if max_gain_pct >= 10.0 and str(row.get("TP 10%", "-")) == "-": df_hist.at[idx, "TP 10%"] = f"✅ {now_date}"
...
def generuj_raport_ai(row_ta, row_ml=None, btc_dom=55.0):
    ...
    support_str, resistance_str, sl_str = str(row_ta.get("Wsparcie", "0.00")), str(row_ta.get("Opór", "0.00")), str(row_ta.get("SL (ATR)", "0.00"))
    edge_status = str(row_ml.get("Ocena Przewagi (Edge)", "-")) if row_ml is not None else "-"
    
    raw_smart_score = row_ml.get("Smart Score (%)", 50.0) if row_ml is not None else 50.0
    smart_score = f"{float(raw_smart_score):.2f}"
    
    prognoza_mc = str(row_ml.get("Prognoza MC (24h)", "-")) if row_ml is not None else "-"
    prob_up = str(row_ml.get("Prawdopodobieństwo", "-")) if row_ml is not None else "-"
    ...
    if "Silny Trend" in regime: 
        ...
        struct_desc = f"Aktywo realizuje obecnie kontrolowane, zdrowe cofnięcie po wcześniejszym rajdzie. Cena (${fmt(price_raw)}) schodzi w okolice poziomów popytowych, ale wciąż utrzymuje się nad kluczową średnią EMA200 (${fmt(ema_raw)}). To klasyczny układ 'kupna na dołku' (buy the dip) w trwającym trendzie wzrostowym."
    elif "Spadkowy" in regime: 
        struct_desc = f"Struktura cenowa znajduje się pod silną presją podaży. Cena (${fmt(price_raw)}) notowana jest poniżej średniej EMA200 (${fmt(ema_raw)}), co oznacza, że kontrolę nad rynkiem przejęły niedźwiedzie. Każde lokalne odbicie może być traktowane przez rynek jako okazja do sprzedaży."
    else: 
...
* **Zachowanie Ceny (Price Action):** {struct_desc}
...
* **Korelacja RSI (Relative Strength Index):**
  * Interwał 1H (`{round(rsi_1h, 1)}`): Lokalny szum / daytrading.
...
with tab5:
    ...
    else: st.info("Brak rozliczonych sygnałów w historii.")
