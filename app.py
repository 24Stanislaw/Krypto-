...
# ==========================================
# LISTA TOKENÓW SPOT (Z KUCOIN DLA JUP)
# ==========================================
TOKENS = [
    ...
    {"symbol": "JUP", "kucoin": "JUP-USDT", "gecko_id": "jupiter-ag"},
...
def fetch_from_kucoin(symbol_pair, interval="1hour"):
  url = f"https://api.kucoin.com/api/v1/market/candles?symbol={symbol_pair}&type={interval}"
...
def get_candles_1h(token_info):
  if token_info.get("kucoin"):
    try:
      df = fetch_from_kucoin(token_info["kucoin"], interval="1hour")
...
def get_candles_1d(token_info):
  if token_info.get("kucoin"):
    try:
      df = fetch_from_kucoin(token_info["kucoin"], interval="1day")
      if not df.empty and len(df) >= 3:
        return df
    except Exception:
      pass
